...
const PROJECT_TYPES = [
  ...
  { value: 'social', label: 'Соцсеть (SNS: VK/Instagram/TikTok)', icon: '🌍' },
            ...
            <div className="mt-8 result-card fade-in">
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-4">
                ...
                <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)' }}>
                  ...
                  <div className="text-xl font-bold" style={{ color: 'white' }}>
                    {result.daysMin} — {result.daysMax} рабочих дней
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl mb-6" style={{ background: 'rgba(56,189,248,0.06)', border: '1px solid rgba(56,189,248,0.15)' }}>
                ...
                <div className="text-sm space-y-1" style={{ color: 'rgba(255,255,255,0.8)' }}>
                  <div>
                    {result.baseHours}ч разработка + {result.testingHours}ч QA ({result.testingPercent}%) ={' '}
                    <strong style={{ color: '#38bdf8' }}>{result.totalHours}ч</strong>
                  </div>
                  <div>
                    {result.totalHours}ч ÷ {result.productiveHoursPerDay} продуктивных часов/день ={' '}
                    <strong style={{ color: '#38bdf8' }}>{result.daysMin} — {result.daysMax} рабочих дней</strong>
                  </div>
                  <div>
                    Ставка {(result.dailyRate || 6000).toLocaleString('ru-RU')} ₽/день +{' '}
                    {formatPrice(result.operationalExpenses)} на инфраструктуру и сервисы
                  </div>
                </div>
              </div>
              ...
              {result.estimationBasis && (
                <div className="p-4 rounded-xl mb-6"
                  ...
                  <div className="text-xs mb-2" style={{ color: 'rgba(255,255,255,0.4)' }}>Основа оценки:</div>
                  <p className="text-sm" style={{ color: 'rgba(255,255,255,0.7)' }}>{result.estimationBasis}</p>
                  {result.guardrailsApplied?.map((item, index) => (
                    <p key={index} className="text-xs mt-2" style={{ color: '#38bdf8' }}>{item}</p>
                  ))}
                </div>
              )}
                ...
                <div className="p-4 rounded-xl mb-6" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                  ...
                  <div className="text-sm" style={{ color: 'rgba(255,255,255,0.6)' }}>
                    ...
                    на срок <strong style={{ color: 'rgba(255,255,255,0.8)' }}>
                      {result.studioComparison.daysMin} — {result.studioComparison.daysMax} рабочих дней
                    </strong>.
                  </div>
                </div>
              )}
